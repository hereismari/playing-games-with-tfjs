...
(function() {
  ...
  window.addEventListener('playtetris', function() {
    if (!state) {
      state = 3;
      (function() {
        var
            ...
            // 1pc = 16px box-shadow:0 0 2pc #000;
            html =
              divStyleMargin + '-14pc -10pc;position:fixed;width:20pc;left:50%;top:50%;font:12px Arial;background:rgba(0,0,0,.8);border-radius:1pc;">' +
            ...
            ;
// ;box-shadow:-2px -2px 8px rgba(0,0,0,.4) inset,0 0 2px #000 inset
        tmp2 = divStyleMargin + '0;width:1pc;height:1pc;float:left;" id="tis-';
        for (i = 220; i < s; i++) {
          if (i % w < 4) {
            html += tmp2 + i + '">' + divEnd;
          }
