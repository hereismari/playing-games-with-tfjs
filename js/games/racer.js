var game = (function(){
    ...
    var resize = function(){
        ...
        $("#screen").css("MozTransform", transform).css("transform", transform).css("WebkitTransform", transform).css({
            top: (scale - 1) * render.height / 2 + 1400,
            left: (scale - 1) * render.width / 2 + ($(window).width() - render.width * scale) / 2 + 180
        });
    };
    ...
    window.addEventListener('playracer', function() {''
      console.log('comeca');
      init();
      ...
      spritesheet.src = "../../imgs/spritesheet.high.png";
    });
